    ...
    public static uint Compute(byte[] input, int offset, int length)
    {
        ...
        if (Sse42.IsSupported)
        {
            ...
            if (Sse42.X64.IsSupported && data.Length > sizeof(ulong))
            {
                ...
                for (var i = 0; i < ulongs.Length; i++) crclong = Sse42.X64.Crc32(crclong, ulongs[i]);
            ...
            else if (data.Length > sizeof(uint))
            {
                ...
                for (var i = 0; i < uints.Length; i++) crcLocal = Sse42.Crc32(crcLocal, uints[i]);
            }

            for (var i = processed; i < data.Length; i++) crcLocal = Sse42.Crc32(crcLocal, data[i]);
        ...
        if (Crc32.IsSupported)
        {
            ...
            if (Crc32.Arm64.IsSupported && data.Length > sizeof(ulong))
            {
                processed = data.Length / sizeof(ulong) * sizeof(ulong);
                var ulongs = MemoryMarshal.Cast<byte, ulong>(data.Slice(0, processed));
                for (var i = 0; i < ulongs.Length; i++) crcLocal = Crc32.Arm64.ComputeCrc32C(crcLocal, ulongs[i]);
            }
            else if (data.Length > sizeof(uint))
            {
                processed = data.Length / sizeof(uint) * sizeof(uint);
                var uints = MemoryMarshal.Cast<byte, uint>(data.Slice(0, processed));
                for (var i = 0; i < uints.Length; i++) crcLocal = Crc32.ComputeCrc32C(crcLocal, uints[i]);
            }

            for (var i = processed; i < data.Length; i++) crcLocal = Crc32.ComputeCrc32C(crcLocal, data[i]);
