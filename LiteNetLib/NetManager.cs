...
public partial class NetManager : IEnumerable<NetPeer>
{
    ...
    private void HandleSimulateLatency(NetPacket packet, IPEndPoint remoteEndPoint)
    {
        ...
        var latency = _randomGenerator.Next(SimulationMinLatency, SimulationMaxLatency);
    ...
    {
        if (SimulatePacketLoss && _randomGenerator.NextDouble() * 100 < SimulationPacketLossChance) _dropPacket = true;
    }
    ...
    private void HandleMessageReceived(NetPacket packet, IPEndPoint remoteEndPoint)
    {
        ...
        var peerFound = remoteEndPoint is NetPeer netPeer || TryGetPeer(remoteEndPoint, out netPeer);
    ...
    internal void CreateReceiveEvent(NetPacket packet, DeliveryMethod method, byte channelNumber, int headerSize,
        NetPeer fromPeer)
    {
        NetEvent evt;
    ...
    {
        var remoteEndPoint = NetUtils.MakeEndPoint(address, port);
